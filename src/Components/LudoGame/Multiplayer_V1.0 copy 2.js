...
import { getFirestore, setDoc, doc, serverTimestamp, onSnapshot, updateDoc } from "firebase/firestore";



import { useEffect, useState } from 'react';

const MultiPlayerGame2 = ({ GameId, CurrentUser, UserData }) => {
    ...
    const unsub = onSnapshot(
        ...
        (doc) => {
            ...
            if(doc.data().PlayerList){
                
                // CheckSyncDiff(player_list, doc.data().PlayerList) 
                // FSupdateTurn(current_turn_index);
            }
            if(_Game.State == "Starting"){
    
                console.log("in if state starting")
                // StartButtonClick();
            ...
            else if(_Game.State == "In Progress"){
    ...
    // useEffect(() =>{
    //     console.log("Game useEffect: ", Game);
    ...
    useEffect(() => {
        ...
        //     console.log("in")
        //     names = {
        //         red_player_name: Game.PlayerNames[0] ? Game.PlayerNames[0] : null,
        //         green_player_name: Game.PlayerNames[1] ? Game.PlayerNames[1] : null,
        //         blue_player_name: Game.PlayerNames[2] ? Game.PlayerNames[2] : null,
        //         yellow_player_name: Game.PlayerNames[3] ? Game.PlayerNames[3] : null,
        //     }
        ...
        $("#dice").click(function () {
            ...
            if(_Game.PlayerIds[current_turn_index] != CurrentUser.uid){
                return
            }
            if (stoptime) {
                startTimer();
        ...
        function updateTurn(cti) {
            ...
            $(".current_turn").text(_Game.CurrentTurn.group);
            $(".red_control").hide();
            $(".yellow_control").hide();
            $(".blue_control").hide();
            $(".green_control").hide();
            ...
            if (CurrentUser && _Game.PlayerIds) {
                $("#dice").attr("disabled", true);
                // if (current_turn.group == 'RED' && CurrentUser.uid == _Game.PlayerIds[0]){ 
                if (_Game.CurrentTurn.group == 'RED' && CurrentUser.uid == _Game.PlayerIds[0]){ 
                ...
                else if (_Game.CurrentTurn.group == 'GREEN' && CurrentUser.uid == _Game.PlayerIds[1]){
                ...
                else if (_Game.CurrentTurn.group == 'BLUE' && CurrentUser.uid == _Game.PlayerIds[2]){
                ...
                else if (_Game.CurrentTurn.group == 'YELLOW' && CurrentUser.uid == _Game.PlayerIds[3]){
...
export default MultiPlayerGame2