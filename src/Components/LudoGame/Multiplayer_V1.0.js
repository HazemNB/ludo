...
import { getFirestore, setDoc, doc, serverTimestamp, onSnapshot, updateDoc, increment, arrayUnion } from "firebase/firestore";



import { useEffect, useState } from 'react';

const MultiPlayerGame = ({ Game, GameId, CurrentUser, UserData }) => {
    ...
    const initFsGame = async ( ) => {
        ...
        if (Game.HostId != CurrentUser.uid) {
            ...
            if (Game.PlayerIds && Game.PlayerIds[1]) {
                ...
                _TurnOrder.push(turn_order_lobby[1]);
            }
            if (Game.PlayerIds && Game.PlayerIds[2]) {
                ...
                _TurnOrder.push(turn_order_lobby[2]);
            }
            if (Game.PlayerIds && Game.PlayerIds[3]) {
                ...
                _TurnOrder.push(turn_order_lobby[3]);
            }
     
            const updatePlayerListRes = await  updateDoc(GameRef, {
    ...
    const LoadStartScreen = () => {
        console.log("Load Start: ", Game)
        $("#red_player_name .cun").val(Game.PlayerNames && Game.PlayerNames[0]);
        $("#green_player_name .cun").val(Game.PlayerNames && Game.PlayerNames[1]);
        $("#yellow_player_name .cun").val(Game.PlayerNames && Game.PlayerNames[2]);
        $("#blue_player_name .cun").val(Game.PlayerNames && Game.PlayerNames[3]);

        $("#red_player_name .cun").prop('disabled', true);
        $("#green_player_name .cun").prop('disabled', true);
        $("#yellow_player_name .cun").prop('disabled', true);
        $("#blue_player_name .cun").prop('disabled', true);

        $("#red_player_name .bot").hide();
        $("#green_player_name .bot").hide();
        $("#yellow_player_name .bot").hide();
        $("#blue_player_name .bot").hide();

        // $("#move-blue1").hide(); hide all that not there?

       
    }


    const HostStartClick = () => { //*********************************************************** */
        // console.log("inittt:    ", init_r)
        ...
        // PLAYER NAMES and bot management***************************************
    ...
    const ResetPlayer = (_Player) => {
        _Player.current_position = 0;
        _Player.status = "home";
        _Player.previous_step = null;
    ...
    function initOld() {
        console.log("Init begin:    ", Game)
        // console.log("turn_oder:", turn_oder)
        // console.log("current_turn_index:", current_turn_index)
        // console.log("current_turn:", current_turn)


        if (turn_oder.length > 0) {
            return;
        }

        
        if (Game.turn_Order > 0) {
            return;
        }
        ...
        // initFsGame(turn_oder, current_turn_index).then((res) => {
        //     if(res){
        //         console.log("INIT UPDATE TURN", Game, Game.CTI, Game.TurnOrder[Game.CTI])
        //         updateTurn(Game.CTI);
    ...
    function init() {
        ...
        if (Game.Status != "Starting") {
            ...
            const updateGameStatusRes = updateDoc(GameRef, {
                State: "Starting",
                syncCounter: increment(1)
            });
    ...
    const ConvertToFireStorePL = (_PL) => {
        var FSPlayerList = {}
        Object.keys(_PL).map((_playerkey) => {

            FSPlayerList[_playerkey] = _PL[_playerkey];

            FSPlayerList[_playerkey].player = String(_PL[_playerkey].player)
            FSPlayerList[_playerkey].previous_step = String(_PL[_playerkey].previous_step)
            FSPlayerList[_playerkey].current_step = String(_PL[_playerkey].current_step)
    ...
    const ConvertFromFireStorePL = (_PL, _FSPL) => {
        Object.keys(_FSPL).map((_FSplayerkey) => {

            if (_FSplayerkey != "player" && _FSplayerkey != "previous_step" && _FSplayerkey != "current_step") {

                _PL[_FSplayerkey] = _FSPL[_FSplayerkey]
    ...
    function updateTurn(cti) {
        ...
        if (Game.RankCount > (Game.TurnOrder.length - 2)) {
            
        // console.log("IN if (rank_count > (turn_oder.length - 2))")
            return 0;
        }
        // if (cti > (turn_oder.length - 1)) {
        if (cti > (Game.TurnOrder.length - 1)) {
            
        // console.log("IN if (cti > (turn_oder.length - 1)) { cti = 0; }")
        ...
        if (Game.TurnOrder[cti].rank > 0) {
            // console.log("IN  if (turn_oder[cti].rank > 0){ ++current_turn_index; }", current_turn_index)
        ...
        //Update Player List and CTI and OR Current Turn Here and then get and reset them  in on doc snapshot |^| 
        // console.log(" current_turn_index = cti; CurrentTurnIndex: ", current_turn_index, "  CTI: ", cti)
        // const updatePlayerListRes = await  updateDoc(GameRef, {
                 
        //     CTI:  cti,
        //     CurrentTurn:  Game.TurnOrder[cti]
        // });
        ...
        // console.log(" current_turn = turn_oder[cti]; TurnOrder: ", turn_oder, "     CurrentTurn: ", current_turn)
        current_turn = turn_oder[cti];
        // console.log("current_player : ", current_turn.group);

        // const updatePlayerListRes = updateDoc(GameRef, {
        //     PlayerList : ConvertToFireStorePL(player_list),
        //     CTI: cti,
        //     CurrentTurn : current_turn
        // });

        // // console.log("oringinal pl before alteration:",player_list)
        // var fspl = ConvertToFireStorePL(player_list);
        // // console.log("fspl: ", fspl);
        // // console.log("returned pl:", ConvertFromFireStorePL(player_list, fspl))
        // // console.log("oringinal pl:",player_list)


        // $(".current_turn").text(current_turn.group);
        $(".current_turn").text(Game.TurnOrder[Game.CTI].group);
        $(".red_control").hide();
        $(".yellow_control").hide();
        $(".blue_control").hide();
        $(".green_control").hide();
        ...
        if (CurrentUser && Game.PlayerIds) {
            $("#dice").attr("disabled", true);
            // if (current_turn.group == 'RED' && CurrentUser.uid == _Game.PlayerIds[0]){ 
            if (Game.TurnOrder[Game.CTI].group == 'RED' && CurrentUser.uid == Game.PlayerIds[0]) {

                $("#dice").attr("disabled", false);
                // $("#dice").show();
                $(".red_control").show();
            }
            // else if (current_turn.group == 'GREEN' && CurrentUser.uid == _Game.PlayerIds[1]){
            else if (Game.TurnOrder[Game.CTI].group == 'GREEN' && CurrentUser.uid == Game.PlayerIds[1]) {
            ...
            // else if (current_turn.group == 'BLUE' && CurrentUser.uid == _Game.PlayerIds[2]){
            else if (Game.TurnOrder[Game.CTI].group == 'BLUE' && CurrentUser.uid == Game.PlayerIds[2]) {

                $("#dice").attr("disabled", false);
                // $("#dice").show();
                $(".blue_control").show();
            }
            // else if (current_turn.group == 'YELLOW' && CurrentUser.uid == _Game.PlayerIds[3]){
            else if (Game.TurnOrder[Game.CTI].group == 'YELLOW' && CurrentUser.uid == Game.PlayerIds[3]) {
        ...
        $(Game.TurnOrder[Game.CTI].players[0].home + " .white-box").css("transform", "scale(1.15)");
        six_count = 0;
    ...
    function FSupdateTurn(cti) {
        // console.log("IN FSupdateTurn(", cti, ")")
        ...
        $(".current_turn").text(Game.CurrentTurn.group);
        $(".red_control").hide();
        $(".yellow_control").hide();
        $(".blue_control").hide();
        $(".green_control").hide();
        ...
        if (CurrentUser && Game.PlayerIds) {
            $("#dice").attr("disabled", true);
            // if (current_turn.group == 'RED' && CurrentUser.uid == _Game.PlayerIds[0]){ 
            if (Game.CurrentTurn.group == 'RED' && CurrentUser.uid == Game.PlayerIds[0]) {
            ...
            // else if (current_turn.group == 'GREEN' && CurrentUser.uid == _Game.PlayerIds[1]){
            else if (Game.CurrentTurn.group == 'GREEN' && CurrentUser.uid == Game.PlayerIds[1]) {
            ...
            // else if (current_turn.group == 'BLUE' && CurrentUser.uid == _Game.PlayerIds[2]){
            else if (Game.CurrentTurn.group == 'BLUE' && CurrentUser.uid == Game.PlayerIds[2]) {
            ...
            // else if (current_turn.group == 'YELLOW' && CurrentUser.uid == _Game.PlayerIds[3]){
            else if (Game.CurrentTurn.group == 'YELLOW' && CurrentUser.uid == Game.PlayerIds[3]) {
    ...
    function getActivePlayers(home) {
        // console.log("IN getActivePlayers Home: ", home)
        ...
        // console.log("ActivePlayers: ", active_players)
    ...
    function getWinPlayers(home) {
        // console.log("IN getWinPlayers Home: ", home)
        ...
        // console.log("win_players: ", win_players)

        return win_players;
    }

    function getHomePlayers(home) {
        // console.log("IN getHomePlayers Home: ", home)
        ...
        // console.log("home_players: ", home_players)

        return home_players;
    }

    const CheckSyncDiff = (localPL, FSPL) => {
        var Diff = {}

        Object.keys(localPL).map((_plKey) => {

            if (localPL[_plKey].current_position != FSPL[_plKey].current_position) {
                // console.log("local PL: ", localPL[_plKey])
                // console.log("Server PL: ", FSPL[_plKey])
            }
        })
    }

    const MoveRemotePlayer = () => {

    }

    var isPlayerIsMoving = false;
    function movePlayer(M_Player, M_Step, M_Road) {
        // Prob gnna need to play around here to update remote player's moves
        // console.log("IN movePlayer M_Player, M_Step, M_Road: ", M_Player, M_Step, M_Road)
            ...
            if (kill_zone.children.length > 1 && !safe_stops.includes(M_Player.path[M_Player.current_position - 1])) {
                ...
                for (let i = 0; i < kill_zone.children.length; i++) {
                    ...
                    if (kill_zone.children[i] != M_Player.player && enemy.color != M_Player.color) {
                        ...
                        enemy = ResetPlayer(enemy);
                        //enemy.resetplayer();    REMEMBER TO TEST THIS SHIT!!!!!! DONEE
    ...
    useEffect(() => {
        ...
        $("#movered1").click(() => {
            console.log("in move red 1 click")
        ...
        $("#dice").click(function () {
            ...
            console.log("in dice click")
    
            if (Game.PlayerIds[current_turn_index] != CurrentUser.uid) {
                return
            }
            if (stoptime) {
                startTimer();
            ...
            <div className="welcome"  >
                ...
                <div className="menu">
                    ...
                    <div className="lobby"  >
                        ...
                        <div className="super-center"><button id="start_btn" onClick={() => HostStartClick()}>Start Game</button><button id="loading_btn" style={{ display: 'none' }} disabled><div className="spinner-border text-danger" role="status">
                            <span className="visually-hidden">Loading...</span>
                        </div></button></div>

                        <span id="waitingForHost" className="super-center">Waiting for host to start...</span>

                    </div>
                    <hr />
                    <div className="super-center">Developed By HB-Tech /&nbsp;<a href="https://www.youtube.com/devninja" className="text-decoration-none" target="_blank">Follow</a></div>
